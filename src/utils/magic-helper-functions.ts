import { baseUrl } from "../services/configs/super-duper-config";
import { TFullOrder, TIngredient, TOrderInfo } from "./data-blueprint";
...
export function request<TResponse>(
  ...
  return fetch(`${baseUrl}${endpoint}`, options).then(checkResponse<TResponse>);
}
...
export function collectOrderIngredients(
  order: TFullOrder | TOrderInfo,
  ingredients: TIngredient[]
): TIngredient[] {
  const orderIngredients: TIngredient[] = [];
  order.ingredients.forEach((idIngredient) => {
    const ingredient = ingredients.find(
      (element) => element._id === idIngredient
    );
    if (ingredient) {
