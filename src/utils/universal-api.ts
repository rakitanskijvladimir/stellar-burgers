import {
  request,
  TBasicResponse,
  TBasicResponseWithMessage,
} from "./magic-helper-functions";
import {
  TForgotPass,
  TIngredient,
  TOrder,
...
): Promise<TBasicResponse & TUserWithTokens> {
...
export function updateUserInfo(
  user: TUserUpdate
): Promise<TBasicResponse & TUserData> {
  return requestWithRefresh(`/auth/user`, {
    method: "PATCH",
    headers: {
      "Content-Type": "application/json",
...
export function forgotPassword(
  form: TForgotPass
): Promise<TBasicResponseWithMessage> {
...
type TRequestWithRefreshInit = RequestInit & {
  headers: { authorization: string };
};

const requestWithRefresh = async <TResponse>(
  endpoint: string,
  options: TRequestWithRefreshInit
) => {
  try {
